...
export class CaptchaMiniGame extends MiniGame {
    constructor() {
        ...
        this.boxes = [];
        this.checked = [];
        this.maxTotalSize = 400;
        this.sideLength = 3;
        this.randomizeAgainAnimationTimer = 0.0;
    }
    prepare() {
    ...
    randomizeBoxes() {
        this.boxes = [];
        this.checked = [];
        let yOffset = (this.maxTotalSize - 400) / 1.5;
    ...
    checkAllSelected() {
        return this.checked.every((checked, i) => {
    ...
    update(state, deltaTime) {
        ...
        for (var i = 0; i < this.boxes.length; i++) {
            ...
            // Draw box border
            this.ctx.beginPath();
            this.ctx.rect(this.boxes[i].x, this.boxes[i].y, this.boxes[i].w, this.boxes[i].h);
