...
export class ModerateMiniGame extends MiniGame {
    ...
    posts: any[] = [];
    randomizePrepareAnimationTimer: number = 0.0;

    hoverPostIndex: number = -1;
    hoverAllow: boolean = false;
    nPosts: number = 3;
    ...
    getRandomOf(entries: string[]): string {
    ...
    genPostText(good: boolean): string {
        let options = [];
        if (good) {
    ...
    drawButton(rect: any, borderColor: string, bgColor: string, hoverFillColor: string, fillColor: string, hovering: boolean, filled: boolean, text: string) {
        // Border
        this.ctx!.beginPath();
        this.ctx!.lineWidth = 2;
        ...
        // Background
        if (hovering) this.ctx!.fillStyle = filled ? fillColor : hoverFillColor;
        else this.ctx!.fillStyle = filled ? fillColor : bgColor;
